...
def detect_face(img, detector_backend = 'opencv', grayscale = False, enforce_detection = True, align = True, max_faces = 1):
	...
	try:
		detected_faces = FaceDetector.detect_face(face_detector, detector_backend, img, align, max_faces)
...
def preprocess_face(img, target_size=(224, 224), grayscale = False, enforce_detection = True, detector_backend = 'opencv', return_region = False, align = True, max_faces = 1):
	...
	for img, region in faces:
		...
		if return_region == True:
			face_imgs.append((img_pixels, region))
		else:
