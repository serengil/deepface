...
def verify():
	...
	#--------------------------
	print("Response : ", resp_obj, "- Type", type(resp_obj))
	print(resp_obj)
	if isinstance(resp_obj,dict) :
		toc =  time.time()
		resp_obj_using_list = dict()
		resp_obj_using_list["pairs"] = list(resp_obj.values())
		resp_obj_using_list["trx_id"] = trx_id
		resp_obj_using_list["seconds"] = toc-tic
		resp_obj_using_list["success"] = True
		return resp_obj_using_list, 200
	else:
		return resp_obj
...
def verifyWrapper(req, trx_id = 0):
	...
	model_name = "VGG-Face"; distance_metric = "cosine"; detector_backend = "opencv"; enforce_detection = True
	...
	if "img" in list(req.keys()):
		...
		for item in raw_content: #item is in type of dict
			...
			if validate_img1 != True or validate_img2 != True:
				return jsonify({'success': False, 'error': 'As imagens devem ser enviadas codificadas em base64 contendo o prefixo URI.', 'error_code': 1}), 400
	...
	if len(instances) == 0:
		return jsonify({'success': False, 'error': 'É preciso pelo menos um par de imagens na requisição', 'error_code': 2}), 400
	...
	try:
		...
		resp_obj = jsonify({'success': False, 'error': str(err), 'error_code': 3}), 400
	return resp_obj
